...
const CustomerProfile = ({ email, onBack }) => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        if (!user) {
          console.warn("Customer not found in customer list");
          setCustomer(null);
          return;
        }
  ...
  return (
    <div className="p-6 bg-[#f5f5f5] min-h-screen">
      <button
        onClick={onBack}
        className="mb-4 text-sm bg-gray-300 hover:bg-gray-400 text-gray-800 py-1 px-3 rounded"
      >
        ← Back to Customers
      </button>

      <h2 className="text-2xl font-bold text-[#2e2e2e] mb-4">Customer Profile</h2>
  ...
  className={`text-sm px-3 py-1 rounded transition ${
    customer.status === "Suspended" ? "bg-green-600 hover:bg-green-700" : "bg-red-600 hover:bg-red-700"
  } text-white ml-2`}
>
          ...
          <tbody>
            {filteredBookings.map((b) => (
              <tr key={b._id} className="border-b hover:bg-gray-100">
                ...
                <td className="px-4 py-2">{b.truckType}</td>
